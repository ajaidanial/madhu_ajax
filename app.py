...
mydb = mysql.connector.connect(
    host="localhost", database="formdata4", user="root", password="pr09r4mm3r"
)
...
def form3():
    ...
    if form.validate_on_submit():
        ...
        for name in data.keys():
            try:  # check if the column_name is already in table
                mycursor.execute(f"SELECT {name} from `form4`;")
                mycursor.fetchone()
            except:  # if error | no column in table | create it | NOTE: varchar(15) => reduced the size for memory overflow
                mycursor.execute(f"ALTER TABLE `form4` add {name} varchar(15);")
        ...
        mycursor.execute(
            f"INSERT INTO form4 ({', '.join(names)}) VALUES {tuple(values)};"
        )
        mydb.commit()
